(function() {
  ...
  async function loadArticles(reset) {
    ...
    data.articles.forEach(function(article) {
      ...
      card.className = 'article-card group block';
      var imgUrl = article.featured_image || img(article.slug, 800, 400);
      var emoji = catEmoji(article.category_name);
      card.innerHTML = '<div style="position:relative;height:200px;background:linear-gradient(135deg,#1e293b,#0f172a);overflow:hidden"><img src="' + imgUrl + '" alt="' + (article.title || '') + '" loading="lazy" style="width:100%;height:100%;object-fit:cover;transition:transform .4s" onerror="this.style.display=\'none\'" class="group-hover:scale-105"><div style="position:absolute;inset:0;background:linear-gradient(180deg,transparent 50%,rgba(0,0,0,.7))"></div><div style="position:absolute;bottom:10px;left:14px;font-size:1.5rem;line-height:1">' + emoji + '</div></div><div style="padding:1.25rem"><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span style="font-size:.75rem;font-weight:500;color:#3b82f6;background:rgba(59,130,246,.1);padding:2px 10px;border-radius:4px;line-height:22px">' + (article.category_name || 'Trending') + '</span><span style="font-size:.75rem;color:#9ca3af">' + (article.reading_time || 5) + ' min</span></div><h3 style="font-weight:600;font-size:.938rem;line-height:1.35;margin-bottom:8px;color:inherit">' + article.title + '</h3><p style="font-size:.875rem;color:#6b7280;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;margin-bottom:12px">' + (article.excerpt || '') + '</p><div style="display:flex;align-items:center;justify-content:space-between;font-size:.75rem;color:#9ca3af"><span>' + formatDate(article.published_at) + '</span><span>' + (article.view_count || 0) + ' views</span></div></div>';
